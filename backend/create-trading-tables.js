...
async function createTables() {
  try {
    ...
    await pool.query('CREATE INDEX IF NOT EXISTS idx_trades_user ON trades(user_id)');
    const result = await pool.query('SELECT COUNT(*) FROM strategies');
    if (parseInt(result.rows[0].count) === 0) {
      await pool.query(`
        INSERT INTO strategies (name, description, tier_required, webhook_token, symbols, win_rate, avg_profit, total_trades)
        VALUES 
          ('Alpha Scalper', 'High-frequency scalping strategy for BTC/USDT with tight stops', 'free', 'alpha_token_001', ARRAY['BTCUSDT'], 68.5, 1.2, 1247),
          ('Trend Master Pro', 'Trend following strategy for major crypto pairs', 'pro', 'trend_token_002', ARRAY['BTCUSDT', 'ETHUSDT', 'SOLUSDT'], 72.3, 2.8, 856),
          ('Swing Elite', 'Multi-timeframe swing trading for maximum gains', 'pro', 'swing_token_003', ARRAY['BTCUSDT', 'ETHUSDT'], 65.2, 4.5, 423),
          ('Quantum Edge', 'AI-powered strategy with dynamic risk management', 'enterprise', 'quantum_token_004', ARRAY['BTCUSDT', 'ETHUSDT', 'BNBUSDT', 'XRPUSDT'], 78.9, 3.2, 2103)
      `);
    }
